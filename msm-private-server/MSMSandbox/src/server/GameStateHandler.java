...
public class GameStateHandler extends BaseClientRequestHandler {
    ...
    public static long getFirstValidUserStructureId(long[] structureIds, Player player, String userGameId) {
	    for (long structureId : structureIds) {
	        JSONObject result = MainExtension.sqlHandler.query(
	            "SELECT user_structure_id FROM player_structures WHERE user_island_id = ? AND structure_id = ? AND user_game_id = ? LIMIT 1",
	...
	private SFSObject bbbIdToFriend(long bbb_id) {
	    ...
	    JSONArray userResult = MainExtension.sqlHandler.query(
	    ...
	    JSONArray outerArray = MainExtension.sqlHandler.query(
	...
	private SFSObject getFriends(User user) {
	    ...
	    // Get friend pairs
	    JSONObject friendReq = MainExtension.sqlHandler.query(
	        "SELECT user_1, user_2 FROM user_friends WHERE user_1 = ? OR user_2 = ?", (long) player.bbb_id, (long) player.bbb_id
	    );
	    ...
	    String idsPlaceholder = friendIds.stream().map(id -> "?").collect(Collectors.joining(","));
	    JSONObject userGameIdRes = MainExtension.sqlHandler.query(
	        "SELECT bbb_id, user_game_id FROM users WHERE bbb_id IN (" + idsPlaceholder + ")", friendIds.toArray()
	    );
	    ...
	    // Batch get nicknames
	    String gameIdsPlaceholder = bbbToGameId.values().stream().map(id -> "?").collect(Collectors.joining(","));
	    JSONObject nickRes = MainExtension.sqlHandler.query(
	        "SELECT id, nickname FROM players WHERE id IN (" + gameIdsPlaceholder + ")", bbbToGameId.values().toArray()
	    );
	...
	public void handleClientRequest(User user, ISFSObject params) {
		...
		case "gs_buy_structure":
			...
			MainExtension.sqlHandler.query(
				...
				player.active_island, params.getInt("structure_id"), user_structure_id, userGameId,
				params.getInt("pos_x"), params.getInt("pos_y"), params.getDouble("scale"),
				params.getInt("flip"), "Structure", serverTime
			);
		...
		case "gs_sell_structure":
	        MainExtension.sqlHandler.query("DELETE FROM player_structures WHERE user_structure_id = ? AND user_game_id = ?", params.getLong("user_structure_id"), userGameId);
		...
		case "gs_move_structure":
			MainExtension.sqlHandler.query("UPDATE player_structures SET pos_x = ? WHERE user_structure_id = ? AND user_game_id = ?", params.getInt("pos_x"), params.getLong("user_structure_id"), userGameId);
			MainExtension.sqlHandler.query("UPDATE player_structures SET pos_y = ? WHERE user_structure_id = ? AND user_game_id = ?", params.getInt("pos_y"), params.getLong("user_structure_id"), userGameId);
		...
		case "gs_flip_structure":
			...
			MainExtension.sqlHandler.query("UPDATE player_structures SET flip = ? WHERE user_structure_id = ? AND user_game_id = ?", params.getBool("flipped") ? 1 : 0, params.getLong("user_structure_id"), userGameId);
			send("gs_update_structure", response2, user);
			response.putBool("success", true);
			send(cmd, response, user);
			break;
		...
		case "gs_buy_egg":
	        ...
	        synchronized (MainExtension.eggLock) {
	            ...
	            MainExtension.sqlHandler.query("INSERT INTO player_eggs (user_egg_id, monster_id, user_game_id, user_island_id) VALUES (?, ?, ?, ?)", egg_id, params.getInt("monster_id"), userGameId, player.active_island);
	        }
		...
		case "gs_hatch_egg":
		    ...
		    JSONObject eggQuery = MainExtension.sqlHandler.query(
		        "SELECT monster_id FROM player_eggs WHERE user_egg_id = ? AND user_island_id = ?", userEggId, player.active_island
		    );
			...
			synchronized (MainExtension.eggLock) {		
				JSONObject result = MainExtension.sqlHandler.query(
				    "SELECT IFNULL(MAX(user_monster_id), -1) + 1 FROM player_monsters WHERE user_game_id = ?", userGameId
				);
				long user_monster_id = result.getJSONArray("result").getJSONArray(0).getLong(0);
				...
				MainExtension.sqlHandler.query(
					"INSERT INTO player_monsters (user_island_id, monster_id, user_monster_id, user_game_id, pos_x, pos_y, muted, flip, name, date_created) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?)",
					player.active_island, monster_id, user_monster_id, userGameId,
					params.getInt("pos_x"), params.getInt("pos_y"),
					params.getInt("flip"), monsterName, serverTime
				);
			...
			break;
		case "gs_sell_egg":			
	        SFSObject priceObject2 = MainExtension.client.getPriceOfMonster(MainExtension.sqlHandler.query(
	        	"SELECT monster_id FROM player_eggs WHERE user_egg_id = ? AND user_island_id = ?", params.getLong("user_egg_id"), player.active_island
	        ...
	        MainExtension.sqlHandler.query("DELETE FROM player_eggs WHERE user_egg_id = ? AND user_island_id = ?", params.getLong("user_egg_id"), player.active_island);
		...
		case "gs_viewed_egg":
			...
			JSONObject queryResult = MainExtension.sqlHandler.query(
			    "SELECT monster_id FROM player_eggs WHERE user_egg_id = ? AND user_island_id = ?", params.getLong("user_egg_id"), player.active_island
			);
		...
		case "gs_mega_monster_message":
			...
			response.putBool("success", true);
			MainExtension.sqlHandler.query("UPDATE player_monsters SET big = ? WHERE user_monster_id = ? AND user_game_id = ?",
				params.containsKey("mega_enable") && params.getBool("mega_enable") ? 1 : 0,
				params.getLong("user_monster_id"), userGameId);
		...
		case "gs_flip_monster":
			response2.putLong("user_monster_id", params.getLong("user_monster_id"));
			response2.putInt("flip", params.getBool("flipped")?1:0);
			MainExtension.sqlHandler.query("UPDATE player_monsters SET flip = ? WHERE user_monster_id = ? AND user_game_id = ?",
				params.getBool("flipped") ? 1 : 0, params.getLong("user_monster_id"), userGameId);
			send("gs_update_monster", response2, user);
			response.putBool("success", true);
			send(cmd, response, user);
			break;
		case "gs_move_monster":
			MainExtension.sqlHandler.query("UPDATE player_monsters SET pos_x = ?, pos_y = ? WHERE user_monster_id = ? AND user_game_id = ?",
				params.getInt("pos_x"), params.getInt("pos_y"), params.getLong("user_monster_id"), userGameId);
		...
		case "gs_sell_monster":
	        SFSObject priceObject3 = MainExtension.client.getPriceOfMonster(MainExtension.sqlHandler.query(
	        	"SELECT monster_id FROM player_monsters WHERE user_monster_id = ? AND user_game_id = ?", params.getLong("user_monster_id"), userGameId
	        ).getJSONArray("result").getJSONArray(0).getInt(0), "coins");
	        ...
	        MainExtension.sqlHandler.query("DELETE FROM player_monsters WHERE user_monster_id = ? AND user_game_id = ?", params.getLong("user_monster_id"), userGameId);
		...
		case "gs_play_scratch_off":
            ...
            int active_island_type = MainExtension.sqlHandler.query(
		...
		case "gs_get_ranked_island_data":
		    ...
		    JSONObject result = MainExtension.sqlHandler.query(
		        "SELECT user_island_id, user_game_id, island_id FROM player_islands WHERE likes > dislikes ORDER BY (likes - dislikes) DESC, likes DESC LIMIT 1 OFFSET ?", rankIndex
		    );
		...
		case "gs_get_random_visit_data":
			data = MainExtension.sqlHandler.query("SELECT user_island_id, user_game_id, island_id FROM player_islands ORDER BY RAND() LIMIT 1").getJSONArray("result");
		...
		case "gs_get_friend_visit_data":
		    ...
		    resultArray = MainExtension.sqlHandler.query("SELECT * FROM player_islands WHERE user_game_id = ?", targetUserGameId).getJSONArray("result");
		    ...
		    JSONArray activeIslandRow = MainExtension.sqlHandler.query(
		...
		case "gs_change_island":
            ...
            MainExtension.sqlHandler.query("UPDATE players SET active_island = ? WHERE id = ?", params.getLong("user_island_id"), userGameId);
            send(cmd, response, user);
			break;
		case "gs_buy_island":
			...
			MainExtension.sqlHandler.query("INSERT INTO player_islands (user_game_id, island_id) VALUES (?, ?)", userGameId, params.getInt("island_id"));
			send(cmd, response, user);
			break;
		case "gs_mute_monster":
	        int muted = new JSONObject(MainExtension.sqlHandler.query("SELECT muted FROM player_monsters WHERE user_monster_id = ? AND user_game_id = ?",
	        		params.getLong("user_monster_id"), userGameId).toString()
    		    	).getJSONArray("result").getJSONArray(0).getInt(0);
	        
	        muted = (muted == 1) ? 0 : 1;
	        
	        MainExtension.sqlHandler.query("UPDATE player_monsters SET muted = ? WHERE user_monster_id = ? AND user_game_id = ?", muted, params.getLong("user_monster_id"), userGameId);
		...
		case "gs_get_code":
		    ...
		    case "info":
		    	...
		    	response.putUtfString("msg", 
			     ...
			     "\nAccounts: " + MainExtension.sqlHandler.query(
		    ...
		    case "help2":
		        ...
		        break;
		    case "login":
		    	JSONArray accountInfo = MainExtension.sqlHandler.query(
		    ...
		    case "refill":
		    	...
		    	break;
		    case "directplace":
		    	MainExtension.sqlHandler.query("UPDATE players SET direct_place = ? WHERE id = ?", player.direct_place == 0 ? 1 : 0, userGameId);
		    ...
		    case "bbs_4":
		        ...
		        String safeTable = args[1].replaceAll("[^a-zA-Z0-9_]", "");
		        MainExtension.sqlHandler.sendCommand("DELETE FROM " + safeTable + ";");
		        break;
		    case "bbs_5":
		    	for (User u : zone.getUserList()) {
		...
		case "gs_rate_island":
			...
			MainExtension.sqlHandler.query("UPDATE player_islands SET " + column + " = " + column + " + 1 WHERE user_island_id = ?", params.getLong("friend_island_id"));
			response.putBool("success", true);
			send(cmd, response, user);
			break;
		...
		case "gs_add_friend":
			...
			JSONArray friend = MainExtension.sqlHandler.query("SELECT * FROM users WHERE bbb_id=?", friendBbbId).getJSONArray("result");
			...
			friend = MainExtension.sqlHandler.query("SELECT * FROM user_friends WHERE (user_1=? AND user_2=?) OR (user_2=? AND user_1=?)", (long) player.bbb_id, friendBbbId, (long) player.bbb_id, friendBbbId).getJSONArray("result");
			
			if (friend.length() == 0) {
				MainExtension.sqlHandler.query("INSERT INTO user_friends (user_1, user_2) VALUES (?, ?)", (long) player.bbb_id, friendBbbId);
			} else {
				response.putBool("success", false);
		...
		case "gs_remove_friend":
			...
			if (friend.length() > 0) {
				MainExtension.sqlHandler.query("DELETE FROM user_friends WHERE (user_1=? AND user_2=?) OR (user_2=? AND user_1=?)", (long) player.bbb_id, friendBbbId, (long) player.bbb_id, friendBbbId);
				response.putBool("success", true);
			} else {
				response.putBool("success", false);
			}
		...
		case "gs_set_displayname":
			...
			MainExtension.sqlHandler.query("UPDATE players SET nickname = ? WHERE id = ?", params.getUtfString("newName"), userGameId);
			send(cmd, response, user);
			break;
		case "gs_set_avatar":
